...
#import "CDWKWebView.h"
#import "WeakScriptMessageDelegate.h"

#define CWINTERFACE_PREFIX @"CWInterface"

@interface CDWKWebView () <WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler>
@end

@implementation CDWKWebView {
...
- (instancetype)init {
    ...
    if (self) {
        ...
        /*! 需要先注册一下这个JS的方法名称。 否则无法响应，  同时实现WKScriptMessageHandler代理
         window.webkit.messageHandlers.CWInterface.postMessage()  // 对应的方法
         */
        config.userContentController = userController;
        [config.userContentController addScriptMessageHandler:[[WeakScriptMessageDelegate alloc] initWithDelegate:self]
                                                         name:CWINTERFACE_PREFIX];
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    ...
    //native执行一行js代码
    [self evaluateJavaScript:@"alert('调用js的弹框')" completionHandler:^(id _Nullable value, NSError *_Nullable error) {
        NSLog(@"%@", value);
    }];
}
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    if ([message.name isEqualToString:CWINTERFACE_PREFIX]) {
        ...
        if ([dataMsgDic[@"hhh"] isEqualToString:@"hhhh"]) {
            ...
            [self evaluateJavaScript:script completionHandler:^(id _Nullable value, NSError *_Nullable error) {
                NSLog(@"%@", value);
            }];
...
- (void)dealloc {
    [[self configuration].userContentController removeScriptMessageHandlerForName:CWINTERFACE_PREFIX];
}
